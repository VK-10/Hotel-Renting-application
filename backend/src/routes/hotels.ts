...
import { BookingType, HotelSearchResponse } from "../shared/type.js";
...
router.get("/search", async (req: Request, res:Response) =>{
    ...
    switch (req.query.sortOption) {
      ...
      case "pricePerNightDesc":
        ...
        const repsonse: HotelSearchResponse = {
            data: hotels,
            pagination : {
                total,
                page: pageNumber,
...
router.post("/:hotelId/bookings", verifytoken, async (req:Request, res:Response) => {
  try {
    ...
    if(paymentIntent.metadata.hotelId !== req.params.hotelId || paymentIntent.metadata.userId !== req.userId) {
      return res.status(400).json({ message : "passage intent mismatch"});
    }
